...
//   const content = r 
  const content = (
    <div>
      <b>{"kk"}</b> people died in <b>{"12"}</b> crashes on{' '}
...
export default function Ap({roads = DATA_URL.ROADS,accidents,year,mapStyle = MAP_STYLE}) {
...
function Roads() {
  ...
  csv(DATA_URL.ACCIDENTS, formatRow, (error, response) => {
    if (!error) {
      return(<App accidents={response} year={response[0].year} />);
    }
  });
}